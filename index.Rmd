...
```{r}
knitr::kable(perf_stan$summary(), digits = 3)
```


We can access the number of posterior draws directly from the model object.
```{r}
ndraws <- posterior::ndraws(perf_stan$draws())
```
...
max_df <- perf_stan$draws(variables = param) %>%
